let state = { tokens: [], alerts: [], narratives: [], stats: {} };
...
function renderTokens() {
  $("#tokens").innerHTML = state.tokens.slice(0, 15).map((t) => `<div class="token-row" data-mint="${esc(t.mint)}">
    <div class="coin"><div class="coin-icon">${esc(t.symbol.slice(0, 2))}</div><div class="coin-name"><strong>${esc(t.name)}</strong><span>${esc(t.symbol)} · ${esc(t.narrative)}</span></div></div>
    ...
    <div class="score ${riskClass(t.risk)}">${t.risk}</div>
  </div>`).join("");
...
function render() { renderStats(); renderTokens(); renderNarratives(); renderAlerts(); }
...
function openToken(mint) {
  ...
  if (t.devHoldingPct >= 12) risks.push("elevated dev holdings"); if (t.top10Pct >= 50) risks.push("holder concentration"); if (t.buyRatio < .43) risks.push("sell-side pressure"); if (t.creatorRisk) risks.push("creator history flag");
  $("#token-detail").innerHTML = `<div class="detail"><span class="kicker">${esc(t.narrative)} // ${esc(t.status)}</span><h2>${esc(t.name)} <span class="risk-low">${esc(t.symbol)}</span></h2><div class="mint">${esc(t.mint)}</div>
    <div class="detail-grid"><div class="detail-card"><label>MOMENTUM</label><strong>${t.momentum}/100</strong></div><div class="detail-card"><label>RISK</label><strong class="${riskClass(t.risk)}">${t.risk}/100</strong></div><div class="detail-card"><label>MARKET CAP</label><strong>${money(t.marketCap)}</strong></div><div class="detail-card"><label>BUYERS</label><strong>${t.uniqueBuyers}</strong></div><div class="detail-card"><label>BUY RATIO</label><strong>${Math.round(t.buyRatio * 100)}%</strong></div><div class="detail-card"><label>SMART WALLETS</label><strong>${t.smartWallets}</strong></div></div>
    <div class="reasons"><div class="reason"><strong>Why it’s moving</strong><br>${esc((momentum.length ? momentum : ["early signal—limited history"]).join(" · "))}</div><div class="reason risk"><strong>Risk read</strong><br>${esc((risks.length ? risks : ["no major heuristic flags"]).join(" · "))}</div></div>
    <div class="detail-actions"><button class="primary" id="export-coin">EXPORT TO OBSIDIAN</button><a href="https://pump.fun/coin/${encodeURIComponent(t.mint)}" target="_blank" rel="noreferrer">PUMP.FUN ↗</a><a href="https://dexscreener.com/solana/${encodeURIComponent(t.mint)}" target="_blank" rel="noreferrer">DEX SCREENER ↗</a></div></div>`;
...
for (const event of ["new-token", "token-update", "alert", "status"]) stream.addEventListener(event, () => refresh());
